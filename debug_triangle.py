...
execution {
    max_steps: 15
    save: [integrated, state, should_flip]
}
'''
    ...
    try:
        ...
        for step in range(min(15, len(result.get('integrated', [])))):
            integrated = result.get('integrated', [0]*15)[step]
            state = result.get('state', [0]*15)[step]
            should_flip = result.get('should_flip', [0]*15)[step]
            print(f"Step {step:2d}: integrated={integrated:5.1f}, state={state:2.0f}, should_flip={should_flip}")
