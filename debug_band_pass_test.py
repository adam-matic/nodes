#!/usr/bin/env python3
"""
Debug script to compare VM output vs expected values for band-pass filter test.
"""
...
from calculate_band_pass_expected import simulate_band_pass_example

def load_test_case():
    """Load the band-pass filter test case."""
    with open('tests/ex26_band_pass_filter_test.json', 'r') as f:
        return json.load(f)
...
def main():
    print("=== Band-Pass Filter Test Debug ===")
    ...
    python_results = simulate_band_pass_example(max_steps=test_case['max_steps'])
    ...
    for i in range(len(expected)):
        ...
        print(f"{i:4d} | {exp_val:10.6f} | {vm_val:18.15f} | {py_val:18.15f} | {vm_diff:13.9f} | {py_vm_diff}")
